...
def run_folder(model, args, config, device, verbose=False):
    ...
    for path in all_mixtures_path:
        ...
        try:
            mix, sr = ffmpeg_load(path, sr=44100, mono=False)
...
def ffmpeg_load(path, sr=22050, mono=False, offset=0.0, duration=None, dtype=np.float32):
    """
    Load an audio file using ffmpeg-python, mimicking librosa.load() behavior.
    This version includes a fix for the read-only array issue and silent execution on Windows.

    Args:
    - path (str): Path to the audio file.
    - sr (int): Target sampling rate. Default is 22050 Hz (librosa default).
    - mono (bool): If True, convert to mono. If False, return stereo if available.
    - offset (float): Start reading after this time (in seconds).
    - duration (float): Only load up to this much audio (in seconds).
    - dtype (numpy.dtype): Desired NumPy data type for the returned array.

    Returns:
    - mix (np.ndarray): Audio time series.
    ...
    try:
        ...
        # Get original channel count and duration
        ...
        # Limit to 2 channels max (like librosa)
        out_channels = 1 if mono else min(2, channels)
        # Determine the appropriate format based on dtype
        if dtype == np.float32:
            format_name = 'f32le'
        ...
        if process.returncode != 0:
            raise RuntimeError(f"FFmpeg command failed: {err.decode()}")

        # Convert the raw audio data to a numpy array
        mix = np.frombuffer(out, dtype).reshape(-1, out_channels)
        # Create a writable copy of the array
        mix = np.array(mix, dtype=np.float32)
        # If mono is True and original audio was not mono, average channels
        if mono and channels != 1:
            mix = np.mean(mix, axis=1, keepdims=True)
        # Normalize to float32 in range [-1, 1] (like librosa)
        if mix.dtype == np.float32:
            np.clip(mix, -1, 1, out=mix)
        elif mix.dtype == np.int16:
            mix = mix.astype(np.float32) / 32768.0
        # Transpose the array to match librosa's output format
        mix = mix.T
        return mix, sr
    except Exception as e:
